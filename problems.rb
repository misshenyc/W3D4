...
def flatten(data)
    ...
    data.inject(flattened){|acc, ele| acc + flatten(ele) }
end

#flattened-top level = [1] + [2] + flatten([[3, 4], [5, [6]]]) + flatten([7,8]) == [1, 2, flatten(..), flatten(7,8)]
#flattened-2nd level =  flatten([[3, 4]) + falatten ([5, [6]]) + [7] + [8]
#flattened-3rd level = [1] + [2] + [3] + [4] + [5] + flatten([6]) +  [7] + [8]
#flattened-4th level = [1] + [2] + [3] + [4] + [5] + [6] + [7] + [8] == [1..8]
